...
def home(request):
    ...
    if request.user.is_authenticated:
        passwordss = Password.objects.all().filter(user=request.user)
        for passwords in passwordss:
            passwords.email = fernet.decrypt(passwords.email.encode()).decode()
            print(passwords.email)
            passwords.password = fernet.decrypt(passwords.password.encode()).decode()
